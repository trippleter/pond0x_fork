...
async function main() {
    ...
    // Tracks my tx's fees and WETH gotten from arbitrage
    ...
    console.log(`\nInitial swapper WETH balance = ${ethers.formatEther(await weth.balanceOf(swapper.target))}\n`)
    const recurSwap = await swapper.recurSwap(poolAddress, whaleBal)
    await logTxData(recurSwap, 'Reccur swapped')
    console.log(`\nAfter-swap swapper WETH balance = ${ethers.formatEther(await weth.balanceOf(swapper.target))}`)
...
async function deployContract(owner, caName, constructor=[]) {
    ...
    console.log(`Deployed ${caName} at ${ca.target}`)
    return ca
}
